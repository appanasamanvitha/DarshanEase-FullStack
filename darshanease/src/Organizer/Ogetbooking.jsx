...
const OrganizerBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:9000/user/getbookings');
  ...
  const deleteBooking = async (bookingId) => {
    try {
      await axios.delete(`http://localhost:9000/organizer/deletebooking/${bookingId}`);
            ...
            {bookings.map((booking, index) => (
              <tr key={booking._id}>
                ...
                <td>
                  ...
                  <Button onClick={() => downloadPDF(booking)} style={{ marginLeft: '10px', color: 'green', background: 'none', border: 'none' }}>
                    <FaDownload />
                  </Button>
                  <Button onClick={() => deleteBooking(booking._id)} style={{ marginLeft: '10px', color: 'red', background: 'none', border: 'none' }}>
                    <FaTrash />
                  </Button>
                </td>
              </tr>
